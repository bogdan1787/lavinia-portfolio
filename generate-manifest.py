...
# Load hashes (contains {hash, w, h} per image — written by optimize-images.py)
try:
    _hashes_raw = json.loads(HASHES_FILE.read_text(encoding="utf-8"))
except Exception:
    _hashes_raw = {}
...
def image_entry(rel: Path, filename: str) -> dict | None:
    ...
    thumb_abs  = Path(__file__).parent / rel.parent / "thumbs" / thumb_name
    if thumb_abs.exists():
        entry["thumb"] = (rel.parent / "thumbs" / thumb_name).as_posix()
...
# Subfolders → categories (skip auto-generated thumbs dirs)
for entry in entries:
    if entry.is_dir() and entry.name.lower() != "thumbs":
