...
def image_entry(rel: Path, filename: str) -> dict | None:
    ...
    # AVIF path — generated by optimize-images.py next to the original
    avif_abs = Path(__file__).parent / rel.parent / (Path(filename).stem + ".avif")
    if avif_abs.exists():
        entry["avif"] = (rel.parent / (Path(filename).stem + ".avif")).as_posix()
